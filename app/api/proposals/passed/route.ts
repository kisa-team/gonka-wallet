...
export const GET = fnDecorator([apiRequestWrapper], async (request: NextRequest) => {
    ...
    const url = `${getChainApiUrl()}/cosmos/gov/v1/proposals?proposal_status=3&pagination.limit=${limit}&pagination.count_total=true&pagination.reverse=true${cursor ? `&pagination.key=${encodeURIComponent(cursor)}` : ""}`;
