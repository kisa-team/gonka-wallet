...
import type { GrantSendTokenPermissionsOptions } from "@/hooks/wallet/useGrantSendTokenPermissions";
import {
    DEFAULT_GAS_LIMIT_GRANT_SEND,
    DEFAULT_GAS_PRICE_GRANT_SEND,
    useGrantSendTokenPermissions,
} from "@/hooks/wallet/useGrantSendTokenPermissions";
import { useWalletStore } from "@/hooks/wallet/useWalletStore";
import { GonkaWallet } from "@/src/utils/wallet/GonkaWallet";

export const GrantSendTokenPermissionsContent: FC = () => {
    ...
    const [gasLimitInput, setGasLimitInput] = useState<string>(
        DEFAULT_GAS_LIMIT_GRANT_SEND.toString()
    );
    const [gasPriceInput, setGasPriceInput] = useState<string>(
        DEFAULT_GAS_PRICE_GRANT_SEND.replace("ngonka", "")
    );
    const [isFeeSettingsOpen, setIsFeeSettingsOpen] = useState<boolean>(false);

    const { grantPermissions, status, transactionHash, error, reset } =
        useGrantSendTokenPermissions();
    ...
    useEffect(() => {
        if (status === "success") {
            ...
            setTimeout(() => {
                reset();
                useWalletStore.getState().closeSheet("grantPermissions");
            }, 2000);
        }
    }, [status, reset]);
    ...
    const handleGrant = async () => {
        if (!GonkaWallet.isValidGonkaAddress(granteeAddress)) {
            addToast({
                title: "Invalid grantee address",
                color: "danger",
                timeout: 3000,
            });
            return;
        ...
        const options: GrantSendTokenPermissionsOptions = {
            gasLimit,
            gasPrice: gasPriceString,
            expirationDays: expirationDaysNum,
        };

        await grantPermissions(granteeAddress, options);
    };

    const isProcessing = status === "signing" || status === "broadcasting" || status === "pending";
    const canGrant =
        !isProcessing && granteeAddress && GonkaWallet.isValidGonkaAddress(granteeAddress);
    const { feeGnk } = parseGasSettings();

    return (
        <div className="flex flex-col gap-4 px-4">
            <div className="text-sm text-zinc-400">
                Grant permissions to allow another address to send tokens from your account on your
                behalf.
            </div>
            <Input
                ...
                onValueChange={setGranteeAddress}
                variant="bordered"
                isDisabled={isProcessing}
                isInvalid={
                    granteeAddress !== "" && !GonkaWallet.isValidGonkaAddress(granteeAddress)
                }
                errorMessage={
                    granteeAddress !== "" && !GonkaWallet.isValidGonkaAddress(granteeAddress)
